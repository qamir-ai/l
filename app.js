...
(() => {
  ...
  const GLOBAL_KNOWLEDGE_KEY = "qamir_ai_global_knowledge_v1";
  const GLOBAL_SETTINGS_KEY = "qamir_ai_global_settings_v1";
  ...
  function readGlobalKnowledge() {
    try {
      const data = JSON.parse(localStorage.getItem(GLOBAL_KNOWLEDGE_KEY) || "[]");
      return Array.isArray(data) ? data : [];
    } catch (e) {
      console.error("Global knowledge read error:", e);
      return [];
    }
  }
  ...
  function writeGlobalKnowledge(items) {
    try {
      localStorage.setItem(GLOBAL_KNOWLEDGE_KEY, JSON.stringify(Array.isArray(items) ? items : []));
      return true;
    } catch (e) {
      console.error("Global knowledge write error:", e);
      return false;
    }
  }
  ...
  function readGlobalSettings() {
    try {
      const data = JSON.parse(localStorage.getItem(GLOBAL_SETTINGS_KEY) || "null");
  ...
  function writeGlobalSettings() {
    try {
      const settings = {
        ...
        mustRules: state.mustRules,
        neverRules: state.neverRules,
        customerRules: state.customerRules,
  ...
  function loadState() {
    try {
      ...
      if (!current) {
        try {
          current = JSON.parse(localStorage.getItem(LEGACY_KEY) || "null");
        } catch (_) {
          current = null;
        }
      }
      ...
      fresh.users = Array.isArray(fresh.users) ? fresh.users : [];
      fresh.sessions = Array.isArray(fresh.sessions) ? fresh.sessions : [];
      fresh.suggestions = Array.isArray(fresh.suggestions) ? fresh.suggestions : [];

      let globalKnowledge = readGlobalKnowledge();

      if (!globalKnowledge.length && current && Array.isArray(current.knowledge) && current.knowledge.length) {
        globalKnowledge = current.knowledge;
        writeGlobalKnowledge(globalKnowledge);
      }

      fresh.knowledge = globalKnowledge;

      const globalSettings = readGlobalSettings();
      if (globalSettings && typeof globalSettings === "object") {
        Object.assign(fresh, globalSettings);
      }
  ...
  function persist() {
    try {
      ...
      toast("Brauzer xotirasiga saqlashda muammo yuz berdi.");
    }
  }
  ...
  function refreshGlobalKnowledge() {
    const knowledge = readGlobalKnowledge();
  ...
  function admin() {
    const u = currentUser();
    return !!u && String(u.username || "").toLowerCase() === "admin";
  }
  ...
  function ensureAdmin() {
    let u = state.users.find(x => String(x.username || "").toLowerCase() === "admin");
    if (!u) {
      u = {
        id: "admin",
        username: "Admin",
  ...
  function splitKnowledgeBlocks(text) {
    ...
    if (marks.length < 2) {
      return [{ text: src, num: marks[0]?.num || "1" }];
    }
    ...
    for (let i = 0; i < marks.length; i++) {
      ...
      if (block) {
        out.push({ text: block, num: marks[i].num });
      }
    }
    return out;
  }
  ...
  function tokenizeKnowledge(text) {
    return [...new Set((normalizeKnowledgeText(text).toLowerCase().match(/[\p{L}\p{N}]{2,}/gu) || []))];
  }

  function stemUz(w) {
    return String(w || "").replace(/(laringiz|laring|lar|ning|dan|dagi|ga|ka|qa|ni|da|de|di|dir|mi|mı|mu|mü|siz|man|men)$/i, "");
  }
  ...
  function similarityScore(query, item) {
    ...
    if (question && (question === qText || qText.includes(question) || question.includes(qText))) {
      score += 100;
    }
    ...
    state.knowledge.filter(k => k.enabled !== false).forEach((k, ki) => {
      ...
      blocks.forEach((b, i) => {
        ...
        items.push({
          k: virtual,
          qa,
          score: similarityScore(query, { k: virtual, qa })
        });
      });
    });
  ...
  function calculateExpression(expression) {
    ...
    let m = s.match(/(-?\d+(?:\.\d+)?)\s*(?:ning)?\s*(\d+(?:\.\d+)?)\s*(?:%|foiz|foizi|foizini)\b/);
    if (m) {
      const base = Number(m[1]);
      const percent = Number(m[2]);
      if (Number.isFinite(base) && Number.isFinite(percent)) {
        return { value: base * percent / 100, expression: `${base} ning ${percent}%` };
      }
    }

    m = s.match(/(\d+(?:\.\d+)?)\s*%\s*(?:of|dan)\s*(\d+(?:\.\d+)?)/);
    if (m) {
      const percent = Number(m[1]);
      const base = Number(m[2]);
      return { value: base * percent / 100, expression: `${percent}% of ${base}` };
    }

    const cleaned = s.replace(/[^0-9+\-*/().%\s]/g, "").trim();
    if (!cleaned) return null;

    const converted = cleaned.replace(/(\d+(?:\.\d+)?)\s*%/g, "($1/100)");
    if (!/^[0-9+\-*/().\s]+$/.test(converted)) {
      return null;
    }

    try {
      const value = Function(`"use strict"; return (${converted})`)();
  ...
  function formatCalculationNumber(value) {
    ...
    return new Intl.NumberFormat("uz-UZ", { maximumFractionDigits: 10 }).format(rounded);
  }
  ...
  function detectCalculation(text) {
    ...
    const natural = raw.match(/(?:hisobla|hisoblab ber|hisob kitob|hisob-kitob|calculate)\s*[:\-]?\s*(.+)$/i);
    if (natural) {
      const result = calculateExpression(natural[1]);
      if (result) return result;
    }

    if (/\d+.*(?:%|foiz|foizi)/i.test(raw)) {
  ...
  function updateHeader() {
    ...
    if ($("topAvatar")) {
      $("topAvatar").innerHTML = u?.avatar && u.avatar !== "assets/avatar.svg"
  ...
  if ($("authForm")) {
    $("authForm").onsubmit = e => {
      ...
      if (un.length < 3) {
        if ($("authError")) $("authError").textContent = "Login kamida 3 belgidan iborat bo‘lsin.";
        return;
      }

      if (pw.length < 6) {
        if ($("authError")) $("authError").textContent = "Parol kamida 6 belgidan iborat bo‘lsin.";
        return;
      }
      ...
      if (authMode === "register") {
        ...
        if (pw !== confirm) {
          if ($("authError")) $("authError").textContent = "Parollar mos emas.";
          return;
        }

        const exists = state.users.some(u => String(u.username || "").toLowerCase() === un.toLowerCase());
        if (exists) {
          if ($("authError")) $("authError").textContent = "Bu login allaqachon mavjud.";
          return;
        }
      ...
      const u = state.users.find(x =>
        String(x.username || "").toLowerCase() === un.toLowerCase() &&
        x.password === pw
      );

      if (!u) {
        if ($("authError")) $("authError").textContent = "Login yoki parol noto‘g‘ri.";
        return;
      }
      ...
      persist();
      showApp();
      toast("Xush kelibsiz, " + u.username + "!");
    };
  }
  ...
  function userSessions() {
    return state.sessions.filter(s => s.userId === state.currentUserId);
  }
  ...
  function renderSessions() {
    ...
    list.innerHTML = arr.length
      ? arr.map(s =>
        `<div class="chat-item ${s.id === state.currentSession ? "active" : ""}" data-session="${esc(s.id)}">
          ${esc(s.title || "Yangi suhbat")}
        </div>`
      ).join("")
      : `<div class="chat-item" style="color:#625a6c">Hozircha suhbat yo‘q</div>`;

    list.querySelectorAll("[data-session]").forEach(x => {
      x.onclick = () => {
  ...
  function addMessage(r, t) {
    ...
    if (r === "user" && s.title === "Yangi suhbat") {
      s.title = t.slice(0, 34) + (t.length > 34 ? "…" : "");
    }
  ...
  function localFallback(t) {
    ...
    // CALCULATOR
    const calc = calculatorAnswer(q);
    if (calc) return calc;

    // GREETING
    if (/^(salom|assalom|assalomu alaykum|hello|hi|hay|qalesan|qalaysan)\b/i.test(q)) {
    ...
    // DEFAULT
    return `Men ${state.agentName || "Qamir"} — sizga yordam berishga tayyorman.\n\nBu savol bo‘yicha hozircha bazamda yetarli aniq ma'lumot yo‘q.`;
  }
...
${relevantContext || "(Mos bilim topilmadi.)"}`;
    ...
    try {
      ...
      console.error("Qamir API error:", error);
      return localFallback(t);
    }
  }
  ...
  function showTyping() {
    ...
    typingEl.innerHTML = `<div class="message assistant">
      <div class="bubble">
        <span class="dot"></span>
        <span class="dot"></span>
        <span class="dot"></span>
  ...
  function openProfile() {
    ...
    if ($("profileBirth")) $("profileBirth").value = u.birthDate || "";
